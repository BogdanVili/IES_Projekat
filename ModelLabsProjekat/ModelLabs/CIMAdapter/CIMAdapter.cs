	...
	public class CIMAdapter
	{
		...
		private bool DoTransformAndLoad(Assembly assembly, ConcreteModel concreteModel, SupportedProfiles extractType, out Delta nmsDelta, out string log)
		{
			...
			try
			{
				...
				switch (extractType)
				{
					case SupportedProfiles.FTN59:
						{
							// transformation to DMS delta					
							TransformAndLoadReport report = SwitchTimeImporter.Instance.CreateNMSDelta(concreteModel);

							if (report.Success)
							{
								nmsDelta = SwitchTimeImporter.Instance.NMSDelta;
								success = true;
							}
							else
							{
							...
							SwitchTimeImporter.Instance.Reset();
